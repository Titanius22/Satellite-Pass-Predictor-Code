 ...
 * 
 * I copied from here
 * orekit-7.0\src\tutorials\java\fr\cs\examples
 */
package Quick_Copy_Files;
 ...
 */
public class AutoconfigurationCustom {

    /** This is a utility class so its constructor is private.
     */
    private AutoconfigurationCustom() {
    }
...
//        final File home    = new File(System.getProperty("user.home"));
        ...
        final String location = (new File(current.getParent())).getParent();
        StringBuffer pathBuffer = new StringBuffer();
//        appendIfExists(pathBuffer, new File(location + "orekit-data.zip"));
...
//        System.setProperty(DataProvidersManager.OREKIT_DATA_PATH, pathBuffer.toString());
        System.setProperty(DataProvidersManager.OREKIT_DATA_PATH, location + "\\orekit-data.zip");
    }
    ...
    private static void appendIfExists(final StringBuffer path, final String directory) {
        try {
            final URL url = AutoconfigurationCustom.class.getClassLoader().getResource(directory);
            if (url != null) {
                if (path.length() > 0) {
                    path.append(System.getProperty("path.separator"));
                }
